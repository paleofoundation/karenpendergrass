...
async function withinRateLimit(redis: Redis, request: Request) {
  const key = `kp-frontier:score-rate:${clientAddress(request)}`;
...
export async function GET() {
  try {
    ...
    const scores = await redis.zrange<StoredRun[]>(SCORE_KEY, 0, 9, { rev: true });
...
export async function POST(request: Request) {
  try {
    const redis = getRedis();
    if (!(await withinRateLimit(redis, request))) {
    ...
    if (callsign.length < 3) return Response.json({ error: 'Callsign must be at least three characters.' }, { status: 400 });
    if (!Number.isFinite(score) || score < 0 || score > 6500) return Response.json({ error: 'Score is outside the expedition range.' }, { status: 400 });
    ...
    return Response.json({ score: record }, { status: 201 });
  } catch (error) {
