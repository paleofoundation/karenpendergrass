...
/**
 * Karen's social profiles — single source of truth, used by the footer and the
 * mobile (hamburger) menu. Also mirror these into JsonLd `sameAs` for the entity graph.
 *
 * NOTE: hrefs below are placeholders pending URL confirmation. Do not deploy with '#'.
 */
export const SOCIAL_LINKS: Social[] = [
  ...
  {
    label: 'Facebook',
    // TODO: replace with Karen's real Facebook page URL (placeholder won't render).
    href: '#',
    icon: (
      <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor" aria-hidden="true">
  ...
  {
    label: 'Instagram',
    href: 'https://www.instagram.com/micrometallomics',
    icon: (
      <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor" aria-hidden="true">
...
export default function SocialLinks({
  ...
  // Don't render links that haven't been wired to a real URL yet.
  const links = SOCIAL_LINKS.filter((s) => s.href && s.href !== '#');
