const testimonials = [
  {
    ...
    name: 'Fred Hart',
    title: 'Founder, Interact (CPG Brand & Packaging Design)',
    context: 'After Karen predicted in 2020 that Pepsi would put prebiotics on their cans, which happened in 2026.',
  },
  {
    quote:
    ...
    <section className="max-w-6xl mx-auto px-6 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {testimonials.map((t, i) => (
          <div
            key={i}
            className="bg-paper-warm rounded-lg border border-ink/5 p-6 flex flex-col"
          >
            <p
              className="text-ink-light leading-relaxed mb-4 flex-1"
            ...
            <div>
              ...
              {t.context && (
                <p className="text-xs text-ink-muted mt-1 italic">{t.context}</p>
              )}
            </div>
          </div>
        ))}
